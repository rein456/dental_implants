...
function initSliders() {
  ...
  if (document.querySelector(".ba__slider")) {
    new Swiper(".ba__slider", {
      // <- Вказуємо склас потрібного слайдера
      // Підключаємо модулі слайдера
      // для конкретного випадку
      modules: [Navigation],
  ...
  function updateCounter(swiper) {
    const counter = swiper.el.querySelector("[data-counter]");
    if (!counter) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  forms.forEach((form) => {
    if (!form.querySelector('[name="name"]') || !form.querySelector('[name="tel"]')) return;
    form.addEventListener("submit", async (e) => {
      ...
      form.reset();
      if (window.flsPopup) {
        window.flsPopup.open("popup_thx");
      }
    });
  });
});
